...
1. Prepara il file di input
	...
	Crea un file di testo (es. esempio_lista.txt) con una CVE per riga:
		CVE-2023-32560
		CVE-2022-1388
		CVE-2021-34527
...
2. Avvia lo script
	...
	python estrai_cve_crawler.py esempio_lista.txt -o output_lista.csv

Opzioni:

	input.txt: percorso del file contenente le CVE

	-o output_lista.csv: nome del file CSV in output (default: output.csv)

---------------------------------------------------------------------------------------------------

Questo script Python estrae automaticamente il **vettore CVSS** e la **descrizione** associati a una lista di CVE,
 facendo scraping direttamente dal sito ufficiale del [National Vulnerability Database (NVD)](https://nvd.nist.gov/).
